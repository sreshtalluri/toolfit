...
- list_contacts: limit extra 2/10

## No-Call Replies

- update_contact (seed 2, other): It seems that the current toolset does not allow for updating phone numbers directly. Please provide another field to update, or let me know if you need assistance with a different request!
...
- list_contacts (seed 2, passed anyway, after 2 regeneration(s)): Both "list_contacts" and "search_contacts" are described identically ("Find contacts") and could return the full contacts list, so it's unclear which one to use.
